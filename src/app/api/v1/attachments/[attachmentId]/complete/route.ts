...
import { completeUpload } from "@/server/files/attachments";
...
export const POST = withApi({}, async ({ actor, params, request, requestId }) => {
  ...
  const result = await completeUpload(actor, access, params.attachmentId!, requestId);
  if (!result.ok) return notFoundProblem(requestId);
  return NextResponse.json({ ok: true, scanStatus: "pending_scan" });
});
