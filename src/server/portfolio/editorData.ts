...
export async function getEditorContext(actor: Actor, tenantId: string) {
  ...
  const pickerObjectives = curriculum
    ? curriculum.domains.flatMap((domain) =>
  ...
  return {
    ...
    options,
    pickerObjectives,
    frameworkLabel: curriculum
      ? `${curriculum.release.frameworkTitle} v${curriculum.release.version}`
      : null,
  };
}
